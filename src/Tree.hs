...
import DataTypes
import GhcPlugins (Name, TyCon)
import Types

type AdjMap = M.Map (DataType Name) (M.Map (K 'L) (S.Set (K 'R)))

type Sol = M.Map RVar (M.Map (DataType Name) (S.Set TyCon))

add :: AdjMap -> AdjMap -> AdjMap
add = M.unionWith (M.unionWith S.union)

diff :: AdjMap -> AdjMap -> AdjMap
diff = M.differenceWith (\l -> Just . M.differenceWith (\l' -> Just . S.difference l') l)

-- flip lift diff
remove :: AdjMap -> Trie -> Trie
remove x (Tip y) = Tip (diff y x)
remove x (Arc ys) = Arc (M.map (remove x) ys)

-- A Trie is satisfied if each Arc with a satisfied adjmap is also satisfied
data Trie
  = Arc (M.Map AdjMap Trie)
  | Tip AdjMap
...
-- Guard a set of constraints by adding a new arc
guardWith :: AdjMap -> Trie -> Trie
guardWith x (Tip y) = Arc (M.singleton x (Tip (diff y x)))
guardWith x (Arc ys) = Arc (M.mapWithKey (guardWith . diff x) ys)

-- Combine two tries
union :: Trie -> Trie -> Trie
union (Tip x) (Tip y) = Tip (add x y)
union (Tip x) (Arc ys) = Arc (M.mapWithKey (union . Tip . diff x) ys)
union (Arc xs) k = Arc (M.mapWithKey (\delta -> union k . remove delta) xs)

-- Take the transitive closure of the trie with initial adjacency map and set of interface nodes
trans :: [RVar] -> Trie -> Trie
trans vs = go M.empty
  where
    go :: Sol -> Trie -> Trie
    go acc (Tip y) = Tip (diff y $ transAdd acc y)
    go acc (Arc ys) = M.foldrWithKey (\y -> union . guardWith (subsol acc y acc) . go (transAdd acc y)) empty ys

    -- Accumalate the transtive closure
    transAdd :: AdjMap -> AdjMap -> AdjMap
    transAdd acc new = _
